...
namespace Swatch {
  ...
  export const createRgbString = (color: tinycolor.Instance | string): Swatch<string> => {
  ...
  export const createHex8String = (color: tinycolor.Instance | string): Swatch<string> => {
    const swatch = new SwatchImp(color);
    return {
...
class SwatchImp implements Swatch<tinycolor.Instance> {
  ...
  constructor(color: tinycolor.Instance | string) {
    ...
    const alphaColors =
      [...Array.from({ length: this.ALPHA_COUNT }, (_, index) => tinycolor.mix(color, tinycolor.names.white, ((index + 1) * 1.0/this.ALPHA_COUNT) * 100)),
        ...Array.from({ length: this.ALPHA_COUNT }, (_, index) => tinycolor.mix(color, tinycolor.names.black, ((index + 1) * 1.0/this.ALPHA_COUNT) * 100))]
    const fallbackColor = tinycolor.mostReadable(color, ['white', 'black'], { includeFallbackColors: true });

    this.color = color;
    this.titleTextColor = alphaColors.find(alphaColor => tinycolor.isReadable(color, alphaColor, { ...this.STANDARDS, size: 'large' })) ?? fallbackColor;
    this.bodyTextColor = alphaColors.find(alphaColor => tinycolor.isReadable(color, alphaColor, { ...this.STANDARDS, size: 'small' })) ?? fallbackColor;
